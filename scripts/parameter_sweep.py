...
def run_parameter_sweep(csv_path):
    print("="*80)
    print("PARAMETER SWEEP OPTIMIZATION - Log-Fib Scalping Strategy")
    print("="*80)
    ...
    for lb, mult, entry, tp, sl in product(lookbacks, multipliers, entry_ratios, tp_ratios, sl_ratios):
        ...
        if result and result['total_trades'] >= 50:  # Minimum trade threshold
            results.append(result)
    
    print()
    print("="*80)
    print(f"OPTIMIZATION COMPLETE - {len(results)} viable configurations found")
    print("="*80)
    
    # Sort by Profit Factor (primary), Win Rate (secondary), Total P&L (tertiary)
    results.sort(key=lambda x: (x['profit_factor'], x['win_rate'], x['total_pnl']), reverse=True)
    ...
    for i, r in enumerate(results[:10], 1):
        print(f"\n#{i} - Profit Factor: {r['profit_factor']:.2f} | Win Rate: {r['win_rate']:.1f}%")
        ...
        print(f"    Trades: {r['total_trades']} | Total P&L: {r['total_pnl']:.5f} | Avg: {r['avg_pnl']:.5f}")
        print(f"    Max Drawdown: {r['max_drawdown']:.5f}")
