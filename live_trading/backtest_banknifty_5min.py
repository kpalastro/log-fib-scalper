"""
═══════════════════════════════════════════════════════════════
BANKNIFTY LOG-FIB STRATEGY TEST - 5 MINUTE TIMEFRAME
═══════════════════════════════════════════════════════════════
    ...
    def load_data(self) -> bool:
        ...
        try:
            ...
            if self.candles:
                ...
                print(f"   Avg candles/day: {len(self.candles) / len(dates):.0f}")
            return True
...
def main():
    print("="*80)
    print("🧪 BANKNIFTY LOG-FIB STRATEGY TEST - 5 MINUTE TIMEFRAME")
    print("="*80)
    print()
    ...
    data_files = list(data_dir.glob("BANKNIFTY*5minute*.csv"))
    
    if not data_files:
        print("❌ No BankNifty 5-minute data found in zerodha_data/")
        print("   Run: python zerodha_data_fetcher.py --symbol BANKNIFTY --interval 5minute")
        return
    ...
    for lookback in [5, 10]:
        print(f"\n{'='*80}")
        print(f"🚀 RUNNING BACKTEST (5-min, Lookback={lookback})")
        print(f"{'='*80}")
        print(f"Strategy: 78.6% Fib Retracement")
        print(f"Lookback: {lookback} candles ({lookback * 5} minutes)")
    ...
    # Save results
    output_file = "backtest_banknifty_5min_comparison.json"
    save_results = {}
    for lb in [5, 10]:
        r = all_results[lb]
        save_results[lb] = {
